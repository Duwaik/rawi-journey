...
### A12. Chapter Completion Overlay Renders on Top of Verdict Card
**Screen:** Verdict card after completing final event of a chapter
**Screenshot:** Image 1 (latest batch) — "PRE-ISLAMIC ARABIA" overlay
sitting directly on top of the Verdict question, answers, and History
Records explanation. Both layers visible simultaneously.

**Problem:** When the user completes the final event of a chapter
(e.g., Event 2 = last Jahiliyyah event), the chapter completion
overlay ("You have witnessed the age before the light") appears ON
TOP of the Verdict card while the answer explanation and History
Records text are still visible behind it. The result is two overlapping
screens — completely unreadable, confusing.

**Fix:** The chapter completion screen must ONLY appear AFTER the user
taps "Continue Journey →" on the Verdict card. Flow:
1. User answers → explanation reveals + VO plays
2. User reads, taps "Continue Journey →"
3. VO stops
4. Verdict card dismisses completely
5. THEN chapter completion cinematic appears (full-screen, clean)
6. User taps to dismiss chapter completion
7. Badge popup (if any)
8. XP popup
9. Navigate to event list

The chapter completion must never share the screen with the Verdict.
These are sequential moments, not simultaneous.

**Priority:** Critical — this is the emotional climax of a chapter
and it's currently broken.

---

### A13. Black Screen on Exit
**Screen:** After tapping Exit in the exit confirmation dialog
**Screenshot:** Image 2 (latest batch) — completely black screen

**Problem:** When the user taps "Exit" in the "Exit game?" dialog,
the screen goes completely black instead of closing the app or
navigating somewhere meaningful.

**Fix:** Check what `Navigator.pop()` or `SystemNavigator.pop()` does
on the exit path. The exit action from the event list should either:
- Close the app entirely (`SystemNavigator.pop()`) — standard for
  games, OR
- If there's a parent route, pop back to it cleanly

If using `SystemNavigator.pop()`, ensure it's called after the dialog
dismisses. The black screen suggests the Navigator stack is empty but
the app process is still alive. Test on the Samsung A56 specifically.

**Priority:** High — users are stuck on a black screen.

---

### A14. Hotspot Progress Not Reflected on Event List Cards
**Screen:** Event list after saving and exiting mid-event

**Problem:** User visits 2 of 4 hotspots in an event, then uses
Save & Exit from the settings overlay. Returns to the event list —
the event card still shows 0/4 hotspot dots, not 2/4. The progress
was saved to SharedPreferences (`hotspot_progress_EVENT_ID`) but the
event list doesn't read it.

**Fix:** In `event_list_screen.dart`, when building the hotspot
progress dots for each event card, check
`PrefsService.loadHotspotProgress(event.id).length` and use that
count for the filled dots. Currently the dots only show progress for
fully completed events.

```dart
// In _buildEventRow or wherever dots are rendered:
final savedProgress = PrefsService.loadHotspotProgress(event.id);
final discoveredCount = savedProgress.length;
// Use discoveredCount for filled dots, totalHotspots for total
```

**Priority:** Medium — progress feels lost without this.

---

### A15. Companion Idle VO Overlaps Branch Card VO
**Screen:** The Crossroads (branch decision card)

**Problem:** When the branch card appears after dismissing The Gate
hotspot, the branch card VO starts playing (600ms delay). But the
idle timer is still running and fires while the user is reading the
branch options. The companion speaks ("Keep exploring!") and its VO
overlaps with the branch card VO. Two audio tracks playing at once.

**Fix:** Cancel the idle timer when the branch card shows. In
`_dismissPanel()`, where `_showBranchCard = true` is set, add
`_idleTimer?.cancel()` immediately before. Also add a guard in
`_onIdle()`:

```dart
void _onIdle() {
  if (!mounted || _phase != _Phase.explore || _activeHotspot != null
      || _showSettings || _showBranchCard) return;  // ← add _showBranchCard
  // ... rest of idle logic
}
```

And restart the idle timer only after the branch choice is made (in
`_onBranchSelected`, which already calls `_resetIdleTimer()`).

**Priority:** High — audio overlap is jarring.

---

### A16. Font Size Change Doesn't Reflect Immediately
**Screen:** Settings → change text size (when B7 accessibility is
implemented) or any pref-based UI change

**Problem:** When the user changes a preference that affects the UI
(font size, language, etc.), the change doesn't reflect on the current
screen or previously visited screens until the app is restarted.

**Fix:** Implement a lightweight refresh mechanism. Two approaches:

**Option A — Rebuild from root (simplest):**
When a UI-affecting preference changes in settings, pop all routes
and rebuild from the event list:
```dart
...
**Option B — ValueNotifier (cleaner):**
Create a `PrefsNotifier` that extends `ValueNotifier`. Settings
screen writes to it, and screens that depend on prefs listen:
```dart
class PrefsNotifier extends ValueNotifier<int> {
  PrefsNotifier() : super(0);
  void notify() => value++;
}
```
Wrap the `MaterialApp` in a `ValueListenableBuilder` that rebuilds
on notify. This handles font scale, language, and any future
pref-driven UI changes.

**Recommendation:** Option B — it's cleaner and scales to any pref
change. Option A works as a quick fix for MVP.

**Priority:** Medium — not blocking until B7 (text scale) ships.

---

### A17. Splash Screen White Squircle Still Present on Resume
**Screen:** App resume after backgrounding

**Problem:** Same issue as A1 — the white rounded square around the
Rawi logo on the Android native splash is still showing on app resume.
Confirming A1 is still outstanding after latest agent push.

**Note for agent:** This is a duplicate confirmation of A1. Same fix
applies — update `LaunchTheme` window background in
`android/app/src/main/res/values/styles.xml` to navy `#0B1E2D`.
Prioritize this — it's been reported twice now.

**Priority:** High — reported multiple times.
...
| # | Item | Ref |
|---|------|-----|
| 1 | Chapter completion overlay on top of Verdict | A12 |
| 2 | VO keeps playing after Continue | A2 |
| 3 | Badge/XP flow interrupts VO | A3 |
| 4 | Companion idle VO overlaps branch card VO | A15 |
| 5 | Character stuck on paths | A8 |
| 6 | Splash white squircle on resume (reported 2x) | A1 / A17 |
| 7 | Black screen on exit | A13 |
| 8 | Skip button not working | A7 |
| 9 | Update content rules — Companions visual prohibition | A11 |

### Phase 2 — UI Bugs
| # | Item | Ref |
|---|------|-----|
| 10 | Event 3 card overflow | A5 |
| 11 | Era auto-collapse | A6 |
| 12 | Path route line cut off | A9 |
| 13 | VO mute button consistency | A10 |
| 14 | Hotspot progress not reflected on event list | A14 |
| 15 | Font size change doesn't reflect immediately | A16 |

### Phase 3 — Enhancements
| # | Item | Ref |
|---|------|-----|
| 16 | Exit dialog redesign | B1 |
| 17 | XP/Badge celebration popup | B2 |
| 18 | Intro cinematic bilingual | B4 |
| 19 | Registration bilingual + mandatory name (2–15 chars) | B5 |
| 20 | Name personalization throughout experience | B5 / C1 |
| 21 | Settings screen overhaul + profile section | B6 / C1 |
| 22 | Badge artwork | B3 |
| 23 | Accessibility text scale | B7 |
