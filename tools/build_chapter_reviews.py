...
REVIEWS = [
    {
        'after': 14,
        'title_en': 'Pre-Prophethood Review — The Path to Light',
        'title_ar': 'مراجعة ما قبل النبوة — الطريق إلى النور',
        'file': 'RAWI_AUDIT_FIXES_PART2.md',
        'format': 'multiline',
        'section_marker': r'#\s*SECTION\s+G\b',
    },
    {
        'after': 47,
...
def find_quiz_section(text, custom_marker=None):
    """Locate the section after 'CHAPTER REVIEW' / 'Chapter Review' / custom marker."""
    if custom_marker:
        m = re.search(custom_marker, text, re.M | re.I)
        if m:
            return text[m.end():]
...
def main():
    ...
    for review in REVIEWS:
        ...
        quiz_text = find_quiz_section(text, custom_marker=review.get('section_marker'))
        if not quiz_text:
